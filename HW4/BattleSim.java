...
public class BattleSim {
    ...
    public static void main(String[] args) {
        ...
        bands.add(new Band("Queen", "Rock", queen));
        bands.add(new Band("BTS", "K-Pop", bts));
        BattleSim sim = new BattleSim(bands);
    ...
    public void addArtist() {
        ...
        if (type.equals("Singer")) {
            ...
            band.getMembers().add(new Singer(name, talent, range));

            System.out.println("Added singer " + name + " to " + band.getBandName());
            ...
            band.getMembers().add(new Guitarist(name, talent, sp));

            System.out.println("Added guitarist " + name + " to " + band.getBandName());
        } else {
     ...
     */
    public void addBand() {
        ArrayList<BandMember> bm = new ArrayList<>();
        System.out.print("What is the band's name? ");
        String name = sc.nextLine();
        System.out.print("Genre: ");
        String genre = sc.nextLine();
        bandList.add(new Band(name, genre, bm));
    ...
    public void battle() {
        ...
        double[] store = {Double.valueOf(b1.perform()), Double.valueOf(b2.perform())};
        if (store[0] > store[1]) {
            winner = b1.getBandName();
            System.out.println("The winner is " + winner);
        } else if (store[0] == store[1]) {
            System.out.println("It was a tie");
        } else {
            winner = b2.getBandName();
            System.out.println("The winner is " + winner);
        }
    }
    ...
    private void listBands() {
        for (int i = 0; i < bandList.size(); i++) {
            System.out.println((i + 1) + ". " + bandList.get(i).getBandName());
        }
    }
